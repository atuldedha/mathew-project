...
const Header = ({ setShowSidebar, username, showSidebar, selected }) => {
  ...
  // state to open account options tab
  const [openAccountTab, setOpenAccountTab] = useState(false);
  return (
    <>
      <div className="flex sticky top-0 z-10 h-[110px] justify-between w-full bg-white1 shadow-shadow2 border-b border-b-gray-300 py-[25px] px-[18px]">
        {/* common for all tabs */}
        ...
        selected === 7 ? (
          ...
          <img
            src={ProfilePicture}
            alt="img"
            className="h-[50px] w-[50px] object-contain rounded-full mr-[40px] hover:cursor-pointer"
            onClick={() => setOpenAccountTab(true)}
          />
        )}
      </div>
      {openAccountTab && (
        <AccountTab setOpenAcccountTab={setOpenAccountTab} data={data} />
      )}
    </>
  );
};
